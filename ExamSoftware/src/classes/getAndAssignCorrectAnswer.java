...
public class getAndAssignCorrectAnswer {
    ...
    public static ArrayList<Question> getAndAssignCorrectAnswer(ArrayList<Question> preguntas) {

        int i = 0;
        File file = new File("parsedAnswers.txt");
        Scanner sc = null;
            ...
            if (line.matches("\\d{1,4}\\.{1}.*")) {
                ...
                if (line.matches("[A-F]\\.{1}.*")) {
                    switch (line.charAt(0)) {
                        case 'A' -> rightAnswers.add(0);
                        case 'B' -> rightAnswers.add(1);
                        case 'C' -> rightAnswers.add(2);
                        case 'D' -> rightAnswers.add(3);
                        case 'E' -> rightAnswers.add(4);
                        case 'F' -> rightAnswers.add(5);
                    }
                }
